...
> 当前环境：本机 HTTPS ComfyUI 0.27.0 已启动；18 项节点和静态枚举预检通过，18/18 项能力均已完成真实 GPU 推理和平台资产回写验收。
...
- 已真实验证：HTTPS CA 信任、节点与静态枚举预检、全部 18 项能力，以及图片、批量图片、文本、GLB 的项目资产登记。
- 验收边界：本轮确认协议、输入语义和输出登记可用；最终生成效果仍需产品和设计人员按真实业务素材持续评审。
...
2026-08-08 实测结果：上述 18 项全部成功。`camera-control-multi` 一次登记 5 张图片；图片理解和文本生成登记 TXT；多视图生三维登记 GLB；其余能力登记 PNG。第二批 8 项在 RTX 3090 上排队执行约 11 分钟，平台 Worker 租约、轮询与输出回写保持正常。
...
2026-08-08 已完成。真实服务适配中发现并修复两项 EasyMark 兼容问题：当前 `sum_stack_flux2_Klein` 要求显式 `context` 输入，不能再依赖 `Anything Everywhere` 隐式广播；编号分区的 `SaveImage` 必须连接采样器生成图 `359.1`，不能保存缩放后的原图 `360.0`。同时移除了两个 API JSON 中仅服务旧前端预览的节点和过期尺寸字段。
