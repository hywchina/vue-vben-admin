...
| 首页 | `/home`（登录默认入口） | Vue、Pinia | 按 0820 需求图保持居中欢迎区和 3×2 六个业务入口；项目创建、最近会话、当前项目和能力可用性继续使用真实平台状态，未配置外部能力不伪造成功；统计与业务数据在项目空间、资产中心和任务中心展示 | `/dashboard` 首页会话与项目上下文 + 项目、资产、应用状态 API |
