...
| 首页 | `/home`（登录默认入口） | Vue、Pinia | 按 0820 需求图保持居中欢迎区和 3×2 六个业务入口；模型训练未接入时展示禁用的参数、图片打标/裁剪和训练选项布局；项目创建、最近会话、当前项目和能力可用性继续使用真实平台状态，未配置外部能力不伪造成功；统计与业务数据在项目空间、资产中心和任务中心展示 | `/dashboard` 首页会话与项目上下文 + 项目、资产、应用状态 API |
...
| 设计生成 | `/design?conversationId=:id` | 独立沉浸式 Vue 页面、Pinia、Canvas、MediaDevices | 左侧历史任务栏可折叠；统一输入器通过可扩展模式目录组织客室零部件、CMF、客室效果和报告能力，三种视觉模式按 0820 顺序显示七个常用工具并将高级能力收进“更多”，仍由平台 API 决定真实可用应用；主要参数在底部快捷区编辑，完整参数由“更多”打开；报告契约未接入时只展示禁用的结构化配置布局；统一资产选择器、图片/Markdown 输入、遮罩和分区编辑、对比、3D、任务状态与会话内深化设计保持原链路；多图片结果使用自适应网格和支持键盘导航的共享全屏查看器 | 设计会话、应用、能力、资产、草稿、任务 API |
