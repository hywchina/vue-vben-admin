...
| 首页 | `/home`（登录默认入口） | Vue、Pinia、响应式 CSS/SVG | 参考甲方 AI 视觉稿建立项目主视觉、四项真实指标、六个业务入口和最近工作区；建筑/客室线稿由本地 SVG/CSS 绘制，不引入外部素材；项目、资产、个人会话、应用、最近项目和任务均直接消费 `/dashboard` 的权限过滤结果并提供真实空状态；模型训练未接入时展示禁用配置布局，报告未接入时明确提示，不伪造外部成功或系统性能数据 | `/dashboard` 聚合 + 项目、资产、应用状态 API |
