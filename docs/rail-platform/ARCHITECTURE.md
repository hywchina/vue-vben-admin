...
| 设计工作台 | `/home`（登录默认入口） | Vue、SVG/CSS 动效与图表、Pinia | 顶部六个快捷入口使用真实项目、会话、资产和能力状态；下方以固定放大圆环展示项目资产、开始设计、AI 应用、任务执行、设计成果和持续复盘的核心闭环；资产、成果和本人任务在所有可访问项目范围聚合，最近项目返回项目级资产类型拆分，最近成果通过 `assetId` 深链切换项目并打开详情 | `/dashboard` 跨项目聚合 API + `GET /assets/:id` + 项目、任务、会话、资产、应用和通知表 |
...
| 设计生成 | `/design?conversationId=:id` | 独立沉浸式 Vue 页面、Pinia、Canvas、MediaDevices | 单一左侧历史栏；统一输入器通过可扩展模式目录组织客室零部件、CMF、客室效果和报告能力，仍由平台 API 决定真实可用应用；主要参数在底部快捷区编辑，完整参数由“更多”打开；统一资产选择器、图片/Markdown 输入、遮罩和分区编辑、对比、3D、任务状态与会话内继续设计保持原链路；多图片结果使用自适应网格和支持键盘导航的共享全屏查看器 | 设计会话、应用、能力、资产、草稿、任务 API |
