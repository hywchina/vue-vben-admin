...
一个应用代表具体功能，用户可以为同一应用新建多个相互独立的应用会话实例。每个实例对应一个独立标签，只允许本实例内同时运行一个任务，并持久保存草稿以及每轮任务的参数、输入、状态和输出；切换标签或刷新后仍可回看并复用。资产流转需要依次选择目标应用、已有或新建的目标会话实例和具体输入位，不下载、不重复上传。工作区文字统一使用 12px 辅助信息、13px 字段标签、14px 正文和 17px 面板标题的层级。带 ComfyUI 图像对比节点的能力会使用业务底图与当前输出提供左右滑动对比。
