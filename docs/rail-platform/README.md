...
`/design` 是脱离后台菜单和多标签外壳的项目沉浸式设计入口：左侧只保留当前用户在当前项目中的历史会话，中央按轮次展示不同工作流的输入、状态和结果，底部是固定的统一输入器。用户未显式选择应用时，输入器展示常用应用快捷入口和“更多”，后台默认按 `text-chat` 文生文执行；选中应用后，其他应用入口收起，只展示“添加输入素材、当前应用、主要参数、高级参数和发送”，完整参数与专用输入位进入抽屉。会话支持新建、重命名和软删除；软删除只隐藏历史入口，不删除项目资产、任务台账或审计记录。由旧应用实例迁移的记录不占用普通历史栏，可从任务中心追溯。18 项原始 ComfyUI API JSON 保持只读。
