...
项目是最高业务边界。普通用户进入项目后从“设计生成”创建项目设计会话；一个会话可以按轮次调用文生文、文生图、遮罩重绘、扩图等多个应用，并持久保存每个应用的参数草稿、输入、任务状态和结果。统一输入器按“客室零部件、CMF、客室效果、报告”业务模式组织后端已发布能力，模式目录只负责筛选、排序和界面语义，不绕开应用与能力 API；缺失能力契约时明确显示“待接入”。同一会话只允许一个任务运行，不同会话可以并行。结果点击“继续设计”后会先由用户明确登记为项目资产，再直接填入本会话的目标应用输入位，不再选择目标应用的第几次会话。产品导航、路由和页面不再提供“应用调试中心”；底层应用目录和适配器配置仍作为“设计生成”的执行能力保留。带 ComfyUI 图像对比节点的能力继续使用业务底图与当前输出提供左右滑动对比；多张图片结果使用网格与连续全屏查看。

登录后的默认首页是“设计工作台”。首页先提供开始新设计、查看我的设计、资产中心、模型训练、报告生成和设计工作台六个快捷入口；新设计使用真实项目接口，历史设计使用真实最近会话，训练与报告缺失能力契约时只显示待接入说明。下方继续用“项目资产 → 开始设计 → AI 应用 → 任务执行 → 设计成果 → 继续设计”的关系图说明平台核心闭环，并用七日任务趋势、任务状态环图、资产构成、最近会话、任务动态和最近成果帮助用户直接继续工作；首页资产与成果按当前账号可访问的全部项目汇总，任务按当前账号在这些项目中的任务汇总，最近项目进一步列出每个项目的资产、任务和资产类型摘要。最近成果可先切换所属项目，再通过带 `assetId` 的地址直接打开具体资产详情。所有统计来自平台 API 的当前真实数据。“项目空间”是独立的单层项目管理入口，纵向展示当前账号可访问的全部项目，不再叠加同名平台概览或顶部当前项目选择器；项目支持搜索、更新时间/创建时间/名称正反序、负责人/管理员修改名称和说明，以及仅对当前用户生效的置顶。项目卡片使用浅色三级文字层级和统一间距，项目编号使用灰色，统计数字强化显示，运行任务大于零时使用平台强调色；卡片主体、四项统计和带悬浮提示的操作按钮均提供清晰入口，并具备整体悬浮、键盘焦点、空列表与禁用状态。卡片只保留一个明确的“开始设计”主操作，不再用重复的三段流程装饰制造入口歧义。普通用户可软删除自己创建且没有活动任务的项目，管理员可软删除任意项目；删除只隐藏项目并登记操作者，不物理删除资产、任务、会话或审计。项目资产、累计任务、运行任务和成员统计均是入口；成员面板显示每位成员的业务 ID 与资产/任务贡献，项目创建者或平台管理员可通过 `USR-*` 邀请成员，并可移除非创建者成员；移除不删除该成员已经形成的资产、任务、会话和审计记录。进入资产中心后，当前项目仍是权限边界；资产可进入持久化多级文件夹，并通过网格/列表视图完成成员筛选、多选取消、移动、独立复制和软删除。复制对象资产会生成新的资产记录与对象键，不与源资产共用可删除对象。应用资产输入选择器复用同一持久化目录，支持逐级进入文件夹、面包屑返回、名称/编号/标签搜索、兼容类型筛选和创建时间/名称/类型排序。任务中心同样提供成员/状态筛选、排序、取消选择和批量软删除；活动任务必须先取消，没有 Worker 执行记录的孤立台账任务会立即取消，真实外部任务则等待 Worker 确认后才能软删除。失败日志以限长摘要显示并可展开详情，台账与审计不会物理删除。
