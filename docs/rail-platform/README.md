...
登录后的默认入口是“首页”。首页参考甲方提供的 AI 视觉稿重组为“项目主视觉、真实数据概览、六项快捷入口、最近工作”四层结构：顶部四项指标来自 `/dashboard` 的当前账号权限汇总；最近项目、最近设计与任务动态均来自真实持久化记录，不展示示例案例、虚构平台动态或没有数据源的 CPU、内存、在线人数。开始新设计、查看我的设计、查看资产中心、开始模型训练、开始报告生成和设计工作台六个入口保持原业务顺序，并在宽屏、平板和窄屏下响应式重排。新设计继续使用真实项目接口，历史设计继续使用真实最近会话；训练与报告缺失能力契约时明确提示待接入，不伪造可用能力。模型训练入口按文档展示参数设置、图片打标/裁剪和底部训练选项，但在 LoRA 服务与数据协议接入前全部禁用。“项目空间”是独立的单层项目管理入口，纵向展示当前账号可访问的全部项目，不再叠加同名平台概览或顶部当前项目选择器；项目支持搜索、更新时间/创建时间/名称正反序、负责人/管理员修改名称和说明，以及仅对当前用户生效的置顶。项目卡片使用浅色三级文字层级和统一间距，项目编号使用灰色，统计数字强化显示，运行任务大于零时使用平台强调色；卡片主体、四项统计和带悬浮提示的操作按钮均提供清晰入口，并具备整体悬浮、键盘焦点、空列表与禁用状态。卡片只保留一个明确的“开始设计”主操作，不再用重复的三段流程装饰制造入口歧义。普通用户可软删除自己创建且没有活动任务的项目，管理员可软删除任意项目；删除只隐藏项目并登记操作者，不物理删除资产、任务、会话或审计。项目资产、累计任务、运行任务和成员统计均是入口；成员面板显示每位成员的业务 ID 与资产/任务贡献，项目创建者或平台管理员可通过 `USR-*` 邀请成员，并可移除非创建者成员；移除不删除该成员已经形成的资产、任务、会话和审计记录。进入资产中心后，当前项目仍是权限边界；资产可进入持久化多级文件夹，并通过网格/列表视图完成成员筛选、多选取消、移动、独立复制和软删除。复制对象资产会生成新的资产记录与对象键，不与源资产共用可删除对象。应用资产输入选择器复用同一持久化目录，支持逐级进入文件夹、面包屑返回、名称/编号/标签搜索、兼容类型筛选和创建时间/名称/类型排序。任务中心同样提供成员/状态筛选、排序、取消选择和批量软删除；活动任务必须先取消，没有 Worker 执行记录的孤立台账任务会立即取消，真实外部任务则等待 Worker 确认后才能软删除。失败日志以限长摘要显示并可展开详情，台账与审计不会物理删除。
