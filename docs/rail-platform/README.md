...
项目是最高业务边界。普通用户进入项目后从“开始设计”创建项目设计会话；一个会话可以按轮次调用文生文、文生图、遮罩重绘、扩图等多个应用，并持久保存每个应用的参数草稿、输入、任务状态和结果。默认能力是 `text-chat` 文生文工作流；同一会话只允许一个任务运行，不同会话可以并行。结果点击“继续设计”后会先由用户明确登记为项目资产，再直接填入本会话的目标应用输入位，不再选择目标应用的第几次会话。应用调试中心和旧应用实例工作区只对管理员开放，用于逐项能力联调。带 ComfyUI 图像对比节点的能力继续使用业务底图与当前输出提供左右滑动对比。
...
`/design` 是脱离后台菜单和多标签外壳的项目沉浸式设计入口：左侧只保留当前用户在当前项目中的历史会话，中央按轮次展示不同工作流的输入、状态和结果，底部是固定的统一输入器。用户未显式选择应用时，输入器展示常用应用快捷入口和“更多”，后台默认按 `text-chat` 文生文执行；选中应用后，其他应用入口收起，输入素材直接预览在文本上方，少量主要参数使用贴近底部栏的快捷弹层原位修改，只有“更多”打开完整参数抽屉。发送后本轮图片输入与文本气泡一起直接显示在时间线，鼠标悬浮输入消息后才出现复制和完整参数入口；任务运行时发送按钮切换为停止按钮。文本工作流结果统一登记为 Markdown 文件并按标题、段落、列表、引用和代码块阅读，结果下方通过带悬浮说明的图标完成复制、下载、加入资产、继续设计和复用运行。会话支持新建、重命名和软删除；软删除只隐藏历史入口，不删除项目资产、任务台账或审计记录。由旧应用实例迁移的记录不占用普通历史栏，可从任务中心追溯。18 项原始 ComfyUI API JSON 保持只读。
