...
本目录是二次开发项目的交接入口。当前交付不是前端 Demo：登录、用户、权限、项目、资产、任务、通知、审计和 AI 助手的本地会话层均使用真实平台 API 与持久化存储。ComfyUI、大模型、LoRA 训练和报告生成等执行能力通过外部服务适配器接入；现有多视图三维工作流属于 18 项 ComfyUI 能力之一。
...
项目是最高业务边界。普通用户进入项目后从“开始设计”创建项目设计会话；一个会话可以在客室零部件、CMF、客室效果三类业务上下文中按轮次调用文生文、文生图、遮罩重绘、扩图等多个应用，并持久保存每个应用的参数草稿、输入、任务状态和结果。业务上下文只负责提示、模板和应用推荐顺序，不复制底层工作流。默认能力是 `text-to-image` 文生图工作流；同一会话只允许一个任务运行，不同会话可以并行。多张图片结果使用网格预览并支持全屏前后浏览。结果点击“加入资产”后可选择当前项目的普通资产目录；点击“深化设计”会先确保结果登记为项目资产，再直接填入本会话的目标应用输入位。产品导航、路由和页面不再提供“应用调试中心”；底层应用目录和适配器配置仍作为“开始设计”的执行能力保留。带 ComfyUI 图像对比节点的能力继续使用业务底图与当前输出提供左右滑动对比。

登录后的默认首页是“设计工作台”。顶部六项快捷入口提供开始新设计、查看我的设计、资产中心、模型训练、报告生成和项目工作台；其中新设计调用真实项目创建 API，个人设计按可访问项目读取真实会话，训练与报告在协议未确定前保持禁用并标记“待接入”。首页继续用“项目资产 → 开始设计 → AI 应用 → 任务执行 → 设计成果 → 深化设计”的关系图说明平台核心闭环，并用七日任务趋势、任务状态环图、资产构成、最近会话、任务动态和最近成果帮助用户直接继续工作；首页资产与成果按当前账号可访问的全部项目汇总，任务按当前账号在这些项目中的任务汇总，最近项目进一步列出每个项目的资产、任务和资产类型摘要。最近成果可先切换所属项目，再通过带 `assetId` 的地址直接打开具体资产详情。所有统计来自平台 API 的当前真实数据。“项目空间”是独立的单层项目管理入口，纵向展示当前账号可访问的全部项目，不再叠加同名平台概览或顶部当前项目选择器；项目支持搜索、更新时间/创建时间/名称正反序、负责人/管理员修改名称和说明，以及仅对当前用户生效的置顶。项目卡片使用浅色三级文字层级和统一间距，项目编号使用灰色，统计数字强化显示，运行任务大于零时使用平台强调色；卡片主体、四项统计和带悬浮提示的操作按钮均提供清晰入口，并具备整体悬浮、键盘焦点、空列表与禁用状态。卡片只保留一个明确的“开始设计”主操作，不再用重复的三段流程装饰制造入口歧义。普通用户可软删除自己创建且没有活动任务的项目，管理员可软删除任意项目；删除只隐藏项目并登记操作者，不物理删除资产、任务、会话或审计。项目资产、累计任务、运行任务和成员统计均是入口；成员面板显示每位成员的业务 ID 与资产/任务贡献，项目创建者或平台管理员可通过 `USR-*` 邀请成员，并可移除非创建者成员；移除不删除该成员已经形成的资产、任务、会话和审计记录。进入资产中心后，当前项目仍是权限边界；资产可进入持久化多级文件夹，并通过网格/列表视图完成成员筛选、多选取消、移动、独立复制和软删除。复制对象资产会生成新的资产记录与对象键，不与源资产共用可删除对象。应用资产输入选择器复用同一持久化目录，支持逐级进入文件夹、面包屑返回、名称/编号/标签搜索、兼容类型筛选和创建时间/名称/类型排序。任务中心同样提供成员/状态筛选、排序、取消选择和批量软删除；活动任务必须先取消，没有 Worker 执行记录的孤立台账任务会立即取消，真实外部任务则等待 Worker 确认后才能软删除。失败日志以限长摘要显示并可展开详情，台账与审计不会物理删除。
...
`/design` 位于统一平台外壳内，继续显示导航、当前项目、通知、用户菜单和全局 AI 助手。页面内部左侧历史会话栏可收起，中央按轮次展示不同工作流的输入、状态和结果，底部是固定统一输入器及客室零部件、CMF、客室效果、报告生成四个业务上下文；前三项已接入真实应用目录，报告生成因模板与服务路线未定而禁用。用户未显式选择应用时，输入器按当前业务上下文展示推荐应用和“更多”，后台默认按 `text-to-image` 文生图执行；选中应用后，其他应用入口收起，主要参数保留在快捷区，“更多”打开完整参数抽屉。每个业务上下文提供首批可见提示模板，模板只向当前业务提示词追加文字。具有媒体字段的应用才显示素材按钮；图片及已登记 Markdown 文本可按对应能力契约读取。发送成功后清空本轮文本和素材，并把本轮图片输入与文本气泡显示在时间线。历史输入图片可进入统一遮罩编辑器；修改重发只替换业务文本，原应用、完整参数、有序资产与遮罩/分区数据作为不可变快照创建新任务。任务运行时只显示动态三点，发送按钮切换为停止按钮；进入或切换会话定位到最新一轮。文本结果按 Markdown 阅读和复制；三维结果通过共享查看器交互查看；两张以上图片以网格显示，点击后可用按钮或方向键前后浏览。结果下方通过图标完成复制、下载、选择项目资产目录入库、深化设计和复用运行。会话支持新建、重命名和软删除；软删除只隐藏历史入口，不删除项目资产、任务台账或审计记录。由旧应用实例迁移的记录不占用普通历史栏，可从任务中心追溯。18 项原始 ComfyUI API JSON 保持只读。

登录后的全部平台页面（包括“开始设计”）右下角均保留同一个全局 AI 设计助手。它支持历史对话、新对话、文本发送、图片/音视频/文档附件、图片预览、文件下载和清空当前对话；助手会话继续按当前用户隔离，不与项目设计会话正文混用。
