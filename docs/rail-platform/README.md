...
客室零部件模式按“文生图、局部重绘、标记修改、多图融合、多角度生成、三维生成、图像放大、图片理解、提示词模板、更多”显示工具；结果提供同序对应的深化操作。多图融合、多角度生成和三维生成既可把当前图片带入编辑框，也可在弹窗中补齐真实工作流所需素材后提交，其中三维生成必须提供前、左、后、右四视图。客室效果模式按“文生图、标记生成、局部重绘、部件/材质融合、平面图填色、环境更改、图像放大、图像理解、提示词模板、更多”显示工具；环境更改复用单图编辑工作流，提供“光环境、窗外环境”专用模板，用户确认图片与文本后才执行。零部件与客室效果模板同样持久化于 PostgreSQL，使用与 CMF 一致的已选标签和管理员维护能力。
