...
平台已通过版本化适配器和独立 Worker 接入 18 项 ComfyUI 工作流，并通过受控业务 API 与同一 Worker 进程接入 AI Toolkit LoRA 训练，以及内置 OOXML/外部 AI 双通道报告生成。LoRA 页面从当前项目选择或上传图片、填写逐图 caption 和白名单参数；报告页面将当前项目文字与 PNG/JPEG 图片编排为可编辑的 DOCX/PPTX，并在任务中标记生成方式。两类产物分别登记为项目 `model` 和 `document` 资产。未配置的外部适配器返回 `ADAPTER_NOT_CONFIGURED`，不会伪造成功结果。详细配置见 [`LORA_TRAINING_INTEGRATION.md`](LORA_TRAINING_INTEGRATION.md) 和 [`REPORT_GENERATION_INTEGRATION.md`](REPORT_GENERATION_INTEGRATION.md)。

项目是最高业务边界。普通用户进入项目后从“设计生成”创建项目设计会话；一个会话可以按轮次调用文生文、文生图、遮罩重绘、扩图等多个应用，并持久保存每个应用在各视觉模式下的参数草稿、输入、任务状态和结果。设计工作区严格保留平台功能侧栏和顶部项目/用户设置栏，在其右侧提供独立可折叠任务栏，形成“平台侧栏 + 任务栏 + 结果/输入区”的 0820 两级结构。统一输入器按“客室零部件、CMF、客室效果、报告”业务模式组织后端已发布能力，三个视觉模式共享后端可见能力目录，模式只负责默认提示词、参数预设、模板和结果操作，不绕开应用与能力 API。任务栏收起后只释放任务栏宽度，不覆盖或移除平台导航；输入器默认显式选中文生图，功能下拉一项对应一个后端 ComfyUI 工作流；右侧显示该工作流的外显参数，“更多”包含当前功能全部参数。管理员在“工作流管理 → 正式能力 → 参数展示”勾选外显字段并调整顺序，配置写入数据库，不改写已发布工作流。草稿按会话、工作流和视觉模式隔离，旧草稿归入客室效果模式。同一会话只允许一个任务运行，不同会话可以并行。结果点击“加入资产”时可选择当前项目的普通资产目录；点击“深化设计”会先确保结果登记为项目资产，再直接填入本会话的目标应用输入位，不再选择目标应用的第几次会话。产品导航、路由和页面不再提供“应用调试中心”；底层应用目录和适配器配置仍作为“设计生成”的执行能力保留。带 ComfyUI 图像对比节点的能力继续使用业务底图与当前输出提供左右滑动对比；多张图片结果使用网格与连续全屏查看。报告模式直接进入 `/report-generation` 专属页面，以真实任务生成 DOCX/PPTX 并回流项目文档资产。

登录成功后固定进入“首页”，不续跳登录前的业务地址。首页采用“项目主视觉、真实数据概览、六项快捷入口”结构：顶部四项指标来自 `/dashboard` 的当前账号权限汇总；六项快捷入口在桌面端按两行三列排列，980px 及以下改为两列、720px 及以下改为单列。首页不再展示最近任务和最近资产。不展示示例案例、虚构平台动态或没有数据源的 CPU、内存、在线人数。开始新设计、查看我的设计、查看资产中心、开始模型训练、开始报告生成和设计工作台六个入口保持原业务顺序，图标与操作统一使用品牌红，服务可用、异常和检测中等状态只在徽标中使用语义色。“查看我的设计”按当前用户权限聚合全部可访问项目中的会话，支持项目、任务或设计名称、开始和结束时间组合检索。模型训练和报告生成分别进入 `/model-training` 与 `/report-generation` 专属页面；LoRA 训练使用真实 AI Toolkit 业务链路，报告生成使用内置 OOXML Worker 创建真实文件和项目资产。平台固定外壳采用“顶部品牌栏 + 全宽标签栏 + 嵌入式竖向导航”结构：红色 Logo 与完整平台名称位于白色顶栏左侧，搜索、通知和用户工具位于右侧；标签栏横跨整个页面；约 184 px 的六项导航从标签栏下方开始，与内容区使用同一画布底色且没有右边框或独立面板背景，不再保留黑色分区。“设计工作台”由原“项目空间”升级而来，继续纵向展示当前账号可访问的全部项目，并在页面内部集中提供平台管理与操作日志入口；用户与权限、工作流管理只对管理员显示，操作日志对所有账号显示但沿用服务端数据范围。项目支持搜索、更新时间/创建时间/名称正反序、负责人/管理员修改名称和说明，以及仅对当前用户生效的置顶。项目卡片使用浅色三级文字层级和统一间距，项目编号使用灰色，统计数字强化显示；项目、资产、累计任务及管理入口使用统一品牌红，运行任务为零时保持中性灰，大于零时使用独立运行状态色。卡片主体、四项统计和带悬浮提示的操作按钮均提供清晰入口，并具备整体悬浮、键盘焦点、空列表与禁用状态。卡片只保留一个明确的“开始设计”主操作，不再用重复的三段流程装饰制造入口歧义。普通用户可软删除自己创建且没有活动任务的项目，管理员可软删除任意项目；删除只隐藏项目并登记操作者，不物理删除资产、任务、会话或审计。项目资产、累计任务、运行任务和成员统计均是入口；成员面板显示每位成员的业务 ID 与资产/任务贡献，项目负责人或平台管理员可通过 `USR-*` 邀请成员、调整编辑/只读角色、移除非负责人成员，并将负责人移交给启用状态的现有成员；原负责人自动转为编辑成员。只读成员可查看项目但不能写入，移除不删除该成员已经形成的资产、任务、会话和审计记录。邀请、角色调整、移除和负责人移交均写入审计，并按用户通知偏好生成站内消息。进入资产中心后，当前项目仍是权限边界；资产可进入持久化多级文件夹，并通过网格/列表视图完成成员筛选、多选取消、移动、独立复制和软删除。复制对象资产会生成新的资产记录与对象键，不与源资产共用可删除对象。应用资产输入选择器复用同一持久化目录，支持逐级进入文件夹、面包屑返回、名称/编号/标签搜索、兼容类型筛选和创建时间/名称/类型排序。任务台账不再占用侧栏入口，用户从设计工作台内各项目的累计任务或运行任务进入 `/jobs`；任务页继续提供成员/状态筛选、排序、取消选择和批量软删除，活动任务必须先取消，没有 Worker 执行记录的孤立台账任务会立即取消，真实外部任务则等待 Worker 确认后才能软删除。失败日志以限长摘要显示并可展开详情，台账与审计不会物理删除。

模型训练和报告生成分别进入 `/model-training` 与 `/report-generation` 专属页面。模型训练已经接入 AI Toolkit 的项目数据集、任务、日志、Loss、停止、checkpoint 和模型资产回流；训练服务未配置或不可达时禁用提交并明确说明。报告生成已经接入项目图片、结构化章节、DOCX/PPTX 输出、任务取消、对象存储和文档资产回流。
...
`ALLOW_SELF_REGISTRATION=true` 时允许内网用户使用用户名、企业邮箱和密码自助注册，新用户固定获得 `user` 角色；设置为 `false` 后只能由管理员在“用户与权限”中创建账号。管理员创建时指定平台角色和初始密码，也可重置其他账号密码并撤销其全部刷新会话；不能在用户列表修改自身角色、状态或密码。登录方式仅保留用户名与密码。
...
`/design` 位于统一平台外壳内，继续显示平台导航、当前项目、通知、用户菜单和全局 AI 助手。页面内部左侧任务栏可独立收起，中央按轮次展示不同应用的输入、状态和结果，底部是固定统一输入器及客室零部件、CMF、客室效果、报告生成四种业务模式。用户未显式选择应用时，输入器按当前模式展示真实可用的快捷能力和“更多”；选中应用后，其他入口收起，主要参数保留在快捷区，完整参数由“更多”打开。图片和已登记的 Markdown 文本资产都可作为输入；发送成功后清空本轮文本和素材，并把本轮输入直接显示在时间线。历史输入支持不可变修改重发，任务运行时显示动态三点并可停止。文本结果按 Markdown 阅读和复制，三维结果通过共享查看器交互查看，多张图片按 2/3/4+ 网格展示并可连续全屏浏览。结果可选择当前项目资产目录入库，也可在同一会话中深化设计；会话支持新建、重命名和软删除，18 项原始 ComfyUI API JSON 保持只读。

CMF 模式默认加载文生图，并按“文生图、局部重绘、多图融合、图像放大、图片理解、提示词模板、更多”显示七项工具。提示词模板采用“颜色、材质、纹样类型”等一级分类和具体参数二级选项，普通用户组合后只填入输入框，全部已选项以可逐项删除的标签显示，管理员可在同一弹层维护分类、选项及顺序。CMF 图片结果按“下载、添加至资产中心、重新绘制、局部重绘、多图融合、图像放大、图像理解”显示操作；下载沿用受控下载接口，图片理解和局部重绘先进入编辑流程，多图融合可转入下方编辑框或在弹窗上传两张参考图并提交，图像放大先确认倍率。历史输入图片主体点击进入普通放大预览，右下角遮罩入口仅在悬浮或键盘聚焦时显示并独立进入遮罩编辑。`jobs.design_mode` 固化历史轮次模式，避免共享工作流在页面刷新后丢失 CMF 语义；模板目录与更新操作者持久化于 PostgreSQL。

客室零部件模式按“文生图、局部重绘、标记修改、多图融合、多角度生成、三维生成、图像放大、图片理解、提示词模板、更多”显示工具；结果提供同序对应的深化操作。多图融合、多角度生成和三维生成既可把当前图片带入编辑框，也可在弹窗中补齐真实工作流所需素材后提交，其中三维生成必须按前、后、左、右顺序提供四视图；工作流会将非方形参考图等比居中裁切后再进入 Hunyuan3D 编码，避免横图被强制拉伸为方形。客室效果模式按“文生图、标记生成、局部重绘、部件/材质融合、平面图填色、环境更改、图像放大、图像理解、提示词模板、更多”显示工具；环境更改复用单图编辑工作流，提供“光环境、窗外环境”专用模板，用户确认图片与文本后才执行。零部件与客室效果模板同样持久化于 PostgreSQL，使用与 CMF 一致的已选标签和管理员维护能力。

选择需要图片输入的工作流后，编辑框上方按真实能力槽位显示图片托盘，页面不再常驻显示左下角加号。用户可一次从本地或当前项目资产中心加入多张图片，在托盘中删除、左右排序或点击图片进入遮罩编辑；本地文件会先登记为当前项目资产。固定需要 N 张图片的工作流会持续显示已上传和剩余数量，素材不足时阻止提交，选择超过 N 张时只接收原选择顺序中的前 N 张并给出提示。有序槽位继续随应用草稿和任务快照持久化，历史轮次图片仍保持主体点击放大、悬浮入口进入遮罩的交互。

登录后的全部平台页面（包括“设计生成”）右下角均保留同一个全局 AI 设计助手。它支持历史对话、新对话、文本发送、图片/音视频/文档附件、图片预览、文件下载和清空当前对话。
...
`AI_ASSISTANT_API_URL` 是 OpenAI Chat Completions 的完整地址。平台按时间顺序发送当前用户最近 50 条已完成消息，实现同一对话内的多轮上下文；最多保留最近 4 张、合计不超过 20 MiB 的图片上下文。图片由平台 API 从私有对象存储读取并转换为 Data URL，因此本机 Docker vLLM 不需要访问浏览器使用的 MinIO 公网地址。非图片附件仍可持久化、预览和下载，但当前 Qwen3-VL 适配器不会伪称读取其正文。
...
请求会携带 `Authorization: Bearer <key>`，API Key 必须与 vLLM 的 `--api-key` 一致。超时、限流、非 2xx 或无效返回都会保存为明确的失败消息，已发送的用户消息不会丢失。
...
登录后从“设计工作台”内部进入 `/audit` 操作日志，所有用户都可进入，但数据范围由后端强制控制：
...
资产中心已按确认的 C 版组织为紧凑生成模块栏、层级目录与图片网格。CMF、零部件、客室、报告分类持久化保存；模糊/精准搜索、任务/类型/成员/日期筛选和正反序由平台 API 执行，详细筛选按需展开。保留全部项目权限聚合、收藏、目录、上传、详情和批量操作。首次更新须执行 `pnpm db:rail:migrate` 应用 `034_asset_generation_categories.sql`；旧数据无法确定模式时进入未分类。方案及实际浏览器验收截图见 [资产中心方案目录](ui-proposals/20260910-assets/)。
