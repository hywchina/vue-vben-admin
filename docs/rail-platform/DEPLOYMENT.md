...
LORA_MODEL_PATH=models/unet/flux-2-klein-9b.safetensors
LORA_VAE_PATH=models/vae/flux2-vae.safetensors
...
开发 API 未设置 `CORS_ALLOWED_ORIGINS`（或留空）时，会在启动时根据本机网卡生成来源白名单，包含 `localhost`、`127.0.0.1`、`[::1]` 和本机 IPv4 地址，协议及端口取自 `APP_PUBLIC_URL`（默认 `http://localhost:5666`）。网卡 IP 变化后需重启 API。不会自动放行其他电脑的 IP、任意域名或不同端口。

如果显式设置 `CORS_ALLOWED_ORIGINS`，只允许逗号分隔的完整来源，不再自动补充地址。例如 `http://localhost:5666,http://127.0.0.1:5666,http://192.168.1.20:5666`，其中 IP 应替换为实际服务器地址。旧 `.env` 若仍只配置 localhost，需清空以使用开发自动发现，或补齐所需来源，然后重启 API。生产环境必须显式配置白名单，不启用网卡自动发现。

通过测试域名访问时，还需把该域名的完整来源加入 API 的 `CORS_ALLOWED_ORIGINS`；Vite `allowedHosts` 仅控制前端 Host，不能替代 API 来源白名单。局域网客户端上传和预览还需配置浏览器可访问的 `S3_PUBLIC_ENDPOINT` 及对象存储 CORS，本次 API 来源修复不自动改变对象存储配置。

可将云端 HTTPS 入口经 FRP 转发到本地 Web 的 5666 端口。Caddy 负责证书与反向代理，FRP 云端业务端口仅监听回环地址；本地原有 `/api` 代理继续连接 5320，不需要额外公开数据库、API 或对象存储管理控制台。

- 在 `apps/web-antd/vite.config.ts` 的 `server.allowedHosts` 中只加入实际受控的测试主机名；不要设为 `true`，也不要使用整个公共域的泛域名放行。现有测试主机名以配置文件为准。
- FRP 应由后台服务管理；本地 Web、API 和所需基础设施仍需持续运行，不能把隧道的自启动当成业务进程的自启动。
